...
- [Development guidelines](#development-guidelines)
    - [General Bitoreum Core](#general-bitoreum-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitoreum Core is a multi-threaded application, and deadlocks or other
...
$ valgrind -v --leak-check=full src/bitoreumd -printtoconsole
```
...
Bitoreum Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Bitoreum Core, where everyone uses
...
pay attention to for reviewers of Bitoreum Core code.

General Bitoreum Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Bitoreum Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Bitoreum Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitoreum-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitoreum-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Bitoreum Core, the wallet was always
