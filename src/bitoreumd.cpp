...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/bitoreum.conf are parsed in qt/dash.cpp's main()
    SetupServerArgs();
#if HAVE_DECL_DAEMON
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  bitoreumd [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see bitoreumd -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for bitoreumd but not for the GUI so do this here
...
#endif
            fprintf(stdout, "Bitoreum Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect bitoreumd signal handlers
    noui_connect();
