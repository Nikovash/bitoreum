...
const char * const BITCOIN_CONF_FILENAME = "bitoreum.conf";
const char * const BITCOIN_PID_FILENAME = "bitoreumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitoreumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitoreumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitoreumCore
    // Mac: ~/Library/Application Support/BitoreumCore
    // Unix: ~/.bitoreumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitoreumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitoreumCore";
#else
    // Unix
    return pathRet / ".bitoreumcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty bitoreum.conf if it does not excist
