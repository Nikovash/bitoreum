...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in bitoreum.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("bitoreum.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in bitoreum.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Bitoreum features */
    { "bitoreum",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "bitoreum",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "bitoreum",               "gobject",                &gobject,                {} },
    { "bitoreum",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
