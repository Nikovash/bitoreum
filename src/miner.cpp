...
void static BitoreumMiner(const CChainParams& chainparams)
{
    LogPrintf("BitoreumMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitoreum-miner");
    ...
    CWallet * pWallet = NULL;
        ...
        if (!EnsureWalletIsAvailable(pWallet, false)) {
            LogPrintf("BitoreumMiner -- Wallet not available\n");
        }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BitoreumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("BitoreumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BitoreumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BitoreumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateBitoreums(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&BitoreumMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
