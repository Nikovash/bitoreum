    ...
    signalled(false)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Bitoreum block chain.").arg(tr(PACKAGE_NAME)) + " " +
...
bool Intro::pickDataDirectory(interfaces::Node& node)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || dataDirDefaultCurrent != dataDirDefaultSettings)
    {
        ...
        intro.setWindowIcon(QIcon(":icons/bitoreum"));
     ...
     * override -datadir in the bitoreum.conf file in the default data directory
     * (to be consistent with bitoreumd behavior)
     */
