...
AC_DEFUN([AX_CXX_COMPILE_STDCXX], [dnl
  ...
  if test x$ac_success = xno; then
    for switch in -std=gnu++$1 -std=gnu++2a -std=gnu++1z -std=gnu++0x; do
  ...
  if test x$ac_success = xno; then
    ...
    for switch in -std=c++$1 -std=c++2a -std=c++1z -std=c++0x +std=c++$1 "-h std=c++$1"; do
      cachevar=AS_TR_SH([ax_cv_cxx_compile_cxx$1_$switch])
      AC_CACHE_CHECK(whether $CXX supports C++$1 features with $switch,
                     $cachevar,
        [ac_save_CXX="$CXX"
...
dnl  Test bodies for checking C++17 and C++20 support
